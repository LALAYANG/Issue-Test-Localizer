...
def read_normalized_occurrence_from_patch_jsonl(patch_path: str) -> int:
    """Reads 'occurrence' field from patch JSONL if available."""
    occ = 0
    try:
        with open(patch_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                try:
                    obj = json.loads(line)
                    for k in ("normalized_occurrence",):
                        if k in obj:
                            occ = max(occ, int(obj[k]))
                except Exception:
                    continue
...
def rank_patches(summary: dict, seed: Optional[int] = None) -> List[List[Dict]]:
    ...
    for pf in patch_files:
        item = {
            ...
            "normalized_occurrence": read_normalized_occurrence_from_patch_jsonl(pf),
            "_rand": random.random(),
        }
        ranked.append(item)
...
def pick_top(ranked_groups: List[List[dict]], seed: Optional[int] = None, pick_one: bool = False) -> Any:
    ...
    if pick_one:
        ...
        max_occ = max(d.get("normalized_occurrence", 0) for d in top_group)
        best = [d for d in top_group if d.get("normalized_occurrence", 0) == max_occ]
        # Break ties randomly
        if seed is not None:
            random.seed(seed)
